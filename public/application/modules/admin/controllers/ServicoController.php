 ...
 * @versão 1.0.1 - 11/08/2014
 */
class Admin_ServicoController extends AdminController {
	/**
	 * Model principal do crud
	 */
	private $model = "Model_Servico";
	...
	public function salvarCadastroAction() {
	    ...
	    if(empty($post["nome"])){
	        ...
	        $querie = "nome = '{$post["nome"]}'";
	        if(!empty($post["codigo"])){
	            $querie.= " AND codigo <> {$post["codigo"]}";
	        }
	        $result = $tbl->fetchRow($querie);
	        if($result){
	            echo Zend_Json_Encoder::encode(array("erro"=>1, "msg"=>"Já existe um serviço cadastrado com este nome em nossa base de dados"));
	            return;
	        }
	    }
	    ...
	    $dLocs = $tblServico->fetchAll(null,"nome ASC");
	    $res = "";
	    if($dLocs->count() > 0) {
	        foreach($dLocs as $d) {
	            $s = "";
	    ...
	    echo Zend_Json_Encoder::encode(array("erro" => $erro, "msg"=>$msg, "codigo" => $codigo, "options"=>$res));
	}
		...
		if(count($itens)>0):	
			foreach($itens as $codigo):	
				if(!empty($codigo) && is_numeric($codigo)) {
        			...
        			$result = $tblVendaProduto->fetchRow("servico = $codigo");
        			if($result){
