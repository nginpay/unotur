function documentos(){
	...
	function uploadGaleria() {
		...
		$("#uploader").pluploadQueue({
			...
			url: _baseUrl+'admin/cliente-documento/upload-galeria',
			chunk_size : '10mb',
			unique_names : true,
	...
	function buscarFotosGaleria() {	
		...
		$.ajax({		
			...
			url: _baseUrl+'admin/cliente-documento/midias', 
			dataType : "html",		
			beforeSend : function() {				
			...
			$("#mensagem").dialog({
				...
				buttons : {
					'Sim apagar' : function() {
						$.ajax({
							 dataType : "json",
					         url: _baseUrl+"admin/cliente-documento/excluir-foto-galeria",
	...
	function editarFotoGaleria(){
		...
		$(".editar-foto").click(function(){
			...
			$.ajax({
		         url: _baseUrl+"admin/cliente-documento/buscar-foto-galeria",
		         data: {codigo: codigo},
		         type: "POST",
		         dataType: 'json',
				 success : function(data) {
	...
	function gravarMidiaGaleria(){
		$.ajax({
	         url: _baseUrl+"admin/cliente-documento/salvar-foto-galeria",
					...
					success : function(data) {						
						if (data.erro == '0') {
							if($("#cadastro-iframe").val()){    								
								window.parent.$('select#cliente').append(data.option);
								window.parent.$('select#cliente').val(data.codigo);								    								
								window.parent.$('select#cliente').change();								
								window.parent.$('select#cliente.chzn-select').trigger("liszt:updated");
